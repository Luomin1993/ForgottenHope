...
public:
        ...
        bool bIsLast)
    {
        if (!isErrorRspInfo(pRspInfo))
        {
            ...
            // 策略交易
                //std::vector<string> TradeIDs;
                //TradeIDs.push("cs1801");
                //TradeIDs.push("CF705");
                //TradeIDs.push("TF1706");
            std::cout << "=====开始进入策略交易=====" << std::endl;
                int LenOfIDs=4;
                //GET_ARRAY_LEN(g_pInstrumentID,LenOfIDs)
                #pragma omp for
                for (int i = 0; i < LenOfIDs; ++i)
                {
              while (loginFlag)
                StrategyCheckAndTrade(g_pInstrumentID[i]);
                }
          }
    }    
    ...
    void StrategyCheckAndTrade(TThostFtdcInstrumentIDType instrumentID)
    {
        ...
        if (priceVec.size() >= 4)
        {
            ...
            // 曙光初现：连续下跌的终点；
            if (priceVec[len - 3] <  priceVec[len - 4] && priceVec[len - 2] < priceVec[len - 3])
                {
                if(KLineVec[len-1].close_price > KLineVec[len-1].open_price)
                                reqOrderInsert(instrumentID, priceVec[len - 1], 1, THOST_FTDC_D_Buy);
            }
                //else if (priceVec[len - 1] < priceVec[len - 2] && priceVec[len - 2] < priceVec[len - 3])
            //  reqOrderInsert(instrumentID, priceVec[len - 1], 1, THOST_FTDC_D_Sell);
        }
    }
