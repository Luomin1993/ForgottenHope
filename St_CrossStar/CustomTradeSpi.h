...
public:
    ...
    void StrategyCheckAndTrade(TThostFtdcInstrumentIDType instrumentID)
    {
        ...
        if (priceVec.size() >= 101)
    	{
    		...
    		//买入信号：看涨十字星
    		if (priceVec[len - 1] < priceVec[len-2] && ( KLineVec[len-1].close_price > KLineVec[len-1].low_price && abs(KLineVec[len-1].close_price - KLineVec[len-1].open_price)<CrossStyle ) && mean100 >= mean20 )
            {    
