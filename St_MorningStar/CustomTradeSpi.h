...
public:
    	...
    	bool bIsLast)
    {
    	if (!isErrorRspInfo(pRspInfo))
    	{
    		...
    		while (loginFlag)
    			StrategyCheckAndTrade(g_pTradeInstrumentID);
    	}
    }    
    ...
    void StrategyCheckAndTrade(TThostFtdcInstrumentIDType instrumentID)
    {
        ...
        if (priceVec.size() >= 4)
    	{
    		...
    		// 早晨之星：连续下跌的终点；
    		if (priceVec[len - 3] <  priceVec[len - 4] && priceVec[len - 2] < priceVec[len - 3] && priceVec[len-1] > priceVec[len-2])
                {
    			if(KLineVec[len-1].close_price > KLineVec[len-1].open_price && KLineVec[len-2].close_price > KLineVec[len-2].open_price)
